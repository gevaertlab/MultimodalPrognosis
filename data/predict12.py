...
from slides import sample_from_patient

import IPython


OUTFILE = "results/mirna_clinical_gene.txt"
print (OUTFILE)
...
class Network(TrainableModel):
    ...
    def forward(self, data, mask):
        ...
        mean = masked_mean((x, y, z), (mask["mirna"], mask["clinical"], mask["gene"]))
        ...
        var = masked_variance((x, y, z), (mask["mirna"], mask["clinical"], mask["gene"])).mean()
        var2 = masked_mean (((x - mean.mean())**2, (y - mean.mean())**2, (z - mean.mean())**2), \
...
class DataGenerator(AbstractPatientGenerator):
    ...
    def sample(self, case, mode='train'):
        ...
        slide_data = sample_from_patient(case, window_size=400, view_size=224, num=10)
        ...
        if mirna_data is None and gene_data is None and clinical_data is None: return None
        if vital_status is None: return None

        return {"mirna": mirna_data, "clinical": clinical_data, "gene": gene_data},\
...
if __name__ == "__main__":
    ...
    model.save("results/predict10.pth") 
