...
#For all four data modalities

MULTIMODAL = sys.argv[1]
OUTFILE = f"results/{MULTIMODAL}_mirna_clinical_gene_slide.txt"
print (OUTFILE)
...
class DataGenerator(AbstractPatientGenerator):
    ...
    def sample(self, case, mode='train'):
        ...
        if MULTIMODAL == 'multi':
            if mode == 'train' and random.randint(1, 3) == 1: clinical_data = None
            if mode == 'train' and random.randint(1, 4) == 1: mirna_data = None
            if mode == 'train' and random.randint(1, 3) == 1: gene_data = None
