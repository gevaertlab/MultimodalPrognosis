...
from data.slides import sample_from_patient

import IPython


OUTFILE = "results/mirna_clinical_gene_slide.txt"
print (OUTFILE)
...
class DataGenerator(AbstractPatientGenerator):
    ...
    def sample(self, case, mode='train'):
        ...
        slides_data = sample_from_patient(case, num=4)
        
        if slides_data is not None:
            slides_data = torch.tensor(slides_data).float()
        ...
        # ADDING MULTIMODAL DROPOUT (randomly dropping one modality):
        ...
        return {"mirna": mirna_data, "clinical": clinical_data, "gene": gene_data, "slides": slides_data},\
...
if __name__ == "__main__":
    ...
    model.save("results/mirna_clinical_gene_slide.pth") 
